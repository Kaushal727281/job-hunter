...
def run_apply(min_score: int, limit: int | None, dry_run: bool):
    ...
    for ats, script in appliers.items():
        ...
        logger.info(f"[{ats}] {len(eligible)} eligible — running {script}")
...
def main():
    ...
    parser.add_argument("--min-score",   type=int, default=7,  help="Apply threshold (default 7)")
    parser.add_argument("--dry-run",     action="store_true",  help="No submissions")
    parser.add_argument("--limit",       type=int,             help="Max jobs per ATS applier")

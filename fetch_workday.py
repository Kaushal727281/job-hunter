...
def fetch_workday_company(company: dict, search_text: str, title_filter: list[str],
    ...
    # ── Fetch individual job descriptions via CXS detail endpoint ────────────
    # The list endpoint returns empty jobDescription; we need per-job detail calls.
    cxs_base = f"https://{tenant}.{wd_instance}.myworkdayjobs.com/wday/cxs/{tenant}/{site_path}"
    for j in jobs:
        if j.get("description"):
            continue
        ext = j.get("wd_ext_path", "")
        if not ext:
            continue
        try:
            detail_url = cxs_base + ext
            dr = sess.get(detail_url, headers={"Accept": "application/json"}, timeout=TIMEOUT)
            if dr.status_code == 200:
                ddata = dr.json()
                # Description is in jobPostingInfo.jobDescription.item (HTML string)
                desc = ""
                jpi = ddata.get("jobPostingInfo") or ddata
                jd  = jpi.get("jobDescription") or {}
                if isinstance(jd, dict):
                    desc = jd.get("item", "") or jd.get("descriptor", "")
                elif isinstance(jd, str):
                    desc = jd
                if not desc:
                    # Some portals put it at top level
                    desc = ddata.get("jobDescription", {}).get("item", "") if isinstance(ddata.get("jobDescription"), dict) else ""
                if desc:
                    # Strip HTML tags for plain-text description
                    import re as _re
                    desc_plain = _re.sub(r"<[^>]+>", " ", desc)
                    desc_plain = _re.sub(r"\s+", " ", desc_plain).strip()
                    j["description"] = desc_plain[:4000]
            time.sleep(0.3)
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument("--profile",  default=os.environ.get("CANDIDATE_PROFILE_SLUG", ""), help="Profile slug to use")
